...
public class RunTest {
	...
	public static void runTest() throws ClassNotFoundException {
		...
		Class<?> testClass = Class.forName("_06_EnumAndAnnotation.Item_35.Sample");
		...
		for (Method m : testClass.getDeclaredMethods()) {
			if (m.isAnnotationPresent(ExceptionTestArrayObject.class)) {
				...
				try {
					...
					Throwable exc = wrappedExc.getCause();
					Class<? extends Exception>[] excTypes = m.getAnnotation(ExceptionTestArrayObject.class).value();
